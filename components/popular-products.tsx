...
export default function PopularProducts() {
  ...
  const getProductColors = (product: Product): ProductColor[] => {
    ...
    if (typeof product.colors === "string") {
      try {
        ...
        console.warn(
          "Failed to parse colors string for product:",
          product.id,
          product.colors
        );
      }
    ...
    if (
      ...
      "name" in product.colors[0] &&
      "value" in product.colors[0]
    ) {
    ...
    if (typeof product.colors[0] === "string") {
      return (product.colors as any[]).map(
  ...
  const getProductBadges = (product: Product): string[] => {
    ...
    if (typeof product.badges === "string") {
      try {
        ...
        if (Array.isArray(parsed)) {
          const validBadges = parsed.filter(
    ...
    const validBadges = product.badges.filter(
      (badge) => badge && typeof badge === "string"
    );
    ...
    if (product.badges.length !== validBadges.length) {
      console.warn(
        "Some invalid badges filtered out for product:",
        product.id,
        "original:",
        product.badges,
        "valid:",
        ...
        <div className="flex flex-row flex-wrap gap-3">
          {products.map((product, index) => {
            ...
            const selectedColorIndex =
              selectedColors[product.id.toString()] || 0;
                  ...
                  {product.sizes && product.sizes.length > 0 && (
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {product.sizes
                          .slice(0, 4)
                          .map((size: string, sizeIndex: number) => (
                            <span
                              key={sizeIndex}
