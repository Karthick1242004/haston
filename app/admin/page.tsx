...
export default function AdminPage() {
  ...
  const getColorName = (
    color: string | { name: string; value: string } | any
  ): string => {
  ...
  const updateNestedSpecification = (
    section: "sizeAndFit" | "materialAndCare",
    field: string,
    value: string
  ) => {
    setSpecifications((prev) => ({
  ...
  const updateProductDetail = (index: number, value: string) => {
    setSpecifications((prev) => ({
      ...prev,
      productDetails: prev.productDetails.map((item, i) =>
        i === index ? value : item
      ),
    }));
  };
  ...
  const removeCareInstruction = (index: number) => {
    if (specifications.materialAndCare.careInstructions.length > 1) {
      setSpecifications((prev) => ({
        ...
        materialAndCare: {
          ...prev.materialAndCare,
          careInstructions: prev.materialAndCare.careInstructions.filter(
            (_, i) => i !== index
          ),
        },
      }));
  ...
  const updateCareInstruction = (index: number, value: string) => {
    setSpecifications((prev) => ({
      ...
      materialAndCare: {
        ...prev.materialAndCare,
        careInstructions: prev.materialAndCare.careInstructions.map((item, i) =>
          i === index ? value : item
        ),
      },
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const hasImages =
      images.length > 0 || (mode === "edit" && existingImages.length > 0);

    // Check if at least one color has a non-default value
    const hasValidColors = productColors.some(
      (c) => c.value && c.value !== "#000000"
    );
    ...
    if (!hasValidColors) {
      console.error(
        "❌ Color validation failed - all colors are default black"
      );
      alert(
        "❌ Color Error: Please select at least one color (change from default black #000000)"
      );
      toast({
        title: "Color Validation Error",
        description:
          "Please select at least one color (change from default black #000000)",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      formData.append(
        "badges",
        JSON.stringify(
          selectedBadges.filter((badge) => badge && typeof badge === "string")
        )
      );
      ...
      if (hasDiscount && discountPercentage) {
        ...
        if (isNaN(discountNum) || discountNum <= 0 || discountNum >= 100) {
          ...
          alert(
            `❌ Discount Error: Please enter a valid discount percentage between 1-99%. Current: ${discountPercentage}`
          );
          toast({
            title: "Discount Validation Error",
            description: `Please enter a valid discount percentage between 1-99%. Current: ${discountPercentage}%`,
            variant: "destructive",
          });
        ...
        console.log("💰 Discount preview:", discountPreview);
        formData.append("discountPercentage", discountPercentage);
        formData.append(
          "originalPrice",
          discountPreview.originalPrice.toString()
        );
      }
      ...
      const cleanedSpecs = {
        ...specifications,
        productDetails: specifications.productDetails.filter(
          (detail) => detail.trim() !== ""
        ),
        materialAndCare: {
          ...specifications.materialAndCare,
          careInstructions:
            specifications.materialAndCare.careInstructions.filter(
      ...
      const url =
        mode === "edit" ? `/api/admin/product/${editId}` : "/api/admin/product";
  ...
  const handleOrderEdit = (order: Order) => {
    ...
    setOrderEditForm({
      ...
      timeline: {
        ...
        processingDays:
          (order as any).timeline?.processingDays || "1-2 business days",
        shippedDays:
          (order as any).timeline?.shippedDays || "3-5 business days",
        deliveredDays:
          (order as any).timeline?.deliveredDays || "5-7 business days",
      },
    });
  ...
  const handleSaveOrder = async () => {
    ...
    if (
      orderEditForm.timeline.processingDays !==
        (currentTimeline.processingDays || "1-2 business days") ||
      orderEditForm.timeline.shippedDays !==
        (currentTimeline.shippedDays || "3-5 business days") ||
      orderEditForm.timeline.deliveredDays !==
        (currentTimeline.deliveredDays || "5-7 business days")
    ) {
                                  ...
                                  onClick={() => {
                                    ...
                                    if (p.specifications) {
                                      setSpecifications({
                                        ...
                                        materialAndCare: {
                                          material:
                                            p.specifications.materialAndCare
                                              ?.material || "",
                                          careInstructions:
                                            p.specifications.materialAndCare
                                              ?.careInstructions &&
                                    ...
                                    if (p.productCategory) {
                                      setMainCategory(
                                        p.productCategory.main || "none"
                                      );
                                      setSubCategory(
                                        p.productCategory.sub || "none"
                                      );
                                    } else {
                                      setMainCategory("none");
                                      setSubCategory("none");
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                        ...
                                        {p.name}"? This action cannot be undone
                                        and will also remove all associated
                                        images from storage.
                                      </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                ...
                <>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="space-y-4 pl-6 border-l-2 border-blue-100">
                          ...
                          {price && discountPercentage && (
                            <div className="bg-blue-50 p-4 rounded-lg">
                              <h4 className="text-sm font-medium text-blue-950 mb-2">
                                Pricing Preview:
                              </h4>
                              <div className="space-y-1 text-sm">
                                <div className="flex justify-between">
                                  ...
                                  <span className="font-medium">
                                    ₹{getDiscountPreview().originalPrice}
                                  </span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="text-gray-600">
                                ...
                                <div className="flex justify-between border-t pt-1">
                                  ...
                                  <span className="text-green-600 font-bold">
                                    ₹{getDiscountPreview().sellingPrice}
                                  </span>
                                </div>
                              </div>
                      ...
                      {mainCategory !== "none" && subCategory !== "none" && (
                        <div className="bg-blue-50 p-3 rounded-lg">
                          <p className="text-sm text-blue-900">
                            ...
                            {getCategoryDisplayName(mainCategory, subCategory)}
                          </p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <label htmlFor="isLook" className="text-sm">
                        Use in Look Breakdown slider
                      </label>
                    </div>
                      ...
                      <div className="flex gap-4">
                        <label className="flex items-center gap-2">
                          ...
                          <span className="text-sm">
                            Single Color (5 images of same color)
                          </span>
                        </label>
                        <label className="flex items-center gap-2">
                          <input
                          ...
                          />
                          <span className="text-sm">
                            Multiple Colors (images must match color order)
                          </span>
                        </label>
                      </div>
                    </div>
                      ...
                      <label className="block text-sm font-medium text-blue-950">
                        ...
                        {colorMode === "multiple" && (
                          <span className="block text-xs text-orange-600 mt-1">
                            ⚠️ Important: Upload images in the EXACT same order
                            as colors below. First color = First set of images,
                            Second color = Second set of images, etc.
                            <br />
                            💡 Tip: Color names are optional - will
                            auto-generate if left empty.
                          </span>
                        )}
                        {colorMode === "single" && (
                          <span className="block text-xs text-blue-600 mt-1">
                            📝 Note: All 5 images should be of the same color
                            specified below.
                            <br />
                            💡 Tip: Color name is optional - will use "Default"
                            if left empty.
                          </span>
                        )}
                      </label>
                    ...
                    <div>
                      <label
                        ...
                        className="block mb-3 text-sm font-medium text-blue-950"
                      >
                        {isLook
                          ? "Slider image (transparent) + 4 additional images"
                          : "Images (up to 5)"}
                        <br />
                        <span className="text-xs text-gray-500">
                          {isLook && "First image should be backgroundless"}
                        </span>
                      </label>
                      <input
                        id="product-images"
                        ...
                        {images.map((file, index) => (
                          <div key={index} className="relative group">
                            <img
                              ...
                              alt="preview"
                              className="h-24 w-24 object-cover border rounded-lg"
                            />
                            <button
                              type="button"
                      ...
                      <div className="mb-6">
                        <label className="block mb-2 text-sm font-medium text-blue-950">
                          ...
                          <span className="text-xs text-gray-500 ml-2">
                            (Click × to remove - images will be deleted from
                            storage)
                          </span>
                        </label>
                        <div className="grid grid-cols-5 gap-2">
                          {existingImages.map((url, idx) => (
                            <div
                              key={idx}
                              className="relative p-2.5 w-auto rounded-sm group"
                            >
                              <img
                                src={url}
                                alt="existing"
                                className="h-24 rounded-sm w-24 object-cover border"
                              />
                              <button
                                type="button"
                                className="absolute top-1 right-1 bg-red-600 hover:bg-red-700 text-white rounded-full w-6 h-6 text-xs font-bold transition-colors shadow-lg opacity-75 group-hover:opacity-100"
                    ...
                    <div className="space-y-6 p-6 bg-gray-50 rounded-lg border">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div>
                          ...
                          <Select
                            value={specifications.waistRise || "none"}
                            onValueChange={(value) =>
                              updateSpecification(
                                "waistRise",
                                value === "none" ? "" : value
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              updateSpecification("features", e.target.value)
                            }
                        ...
                        <div>
                          ...
                          <Select
                            value={specifications.length || "none"}
                            onValueChange={(value) =>
                              updateSpecification(
                                "length",
                                value === "none" ? "" : value
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              updateSpecification("closure", e.target.value)
                            }
                        ...
                        <div>
                          ...
                          <Select
                            value={specifications.flyType || "none"}
                            onValueChange={(value) =>
                              updateSpecification(
                                "flyType",
                                value === "none" ? "" : value
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="Zipper Fly">
                                Zipper Fly
                              </SelectItem>
                              <SelectItem value="Button Fly">
                                Button Fly
                              </SelectItem>
                            </SelectContent>
                          </Select>
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mb-3">
                          Add bullet point descriptions for the product
                        </p>
                        <div className="space-y-2">
                          {specifications.productDetails.map(
                            (detail, index) => (
                              <div key={index} className="flex gap-2">
                                <Input
                                  value={detail}
                                  onChange={(e) =>
                                    updateProductDetail(index, e.target.value)
                                  }
                      ...
                      <div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              {specifications.materialAndCare.careInstructions.map(
                                (instruction, index) => (
                                  <div key={index} className="flex gap-2">
                                    <Input
                                      value={instruction}
                    ...
                    <div>
                      <Button
                        ...
                        className="bg-blue-950 text-white rounded-none w-full py-3"
                      >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                  <div>
                                    <p className="text-red-800">
                                      ...
                                      {(order as any).refundDetails.refund_id}
                                    </p>
                                    <p className="text-red-800">
                                      <strong>Amount:</strong> ₹
                                      {(order as any).refundDetails.amount}
                                    </p>
                                  </div>
                                  <div>
                                    <p className="text-red-800">
                                      <strong>Status:</strong>{" "}
                                      {(order as any).refundDetails.status}
                                    </p>
                                    <p className="text-red-800">
                    ...
                    <div className="space-y-6 mt-6">
                      {isEditingOrder ? (
                        ...
                        <div className="space-y-6">
                              ...
                              <CardContent className="p-4">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    >
                                      ₹
                                      {selectedOrder.orderSummary.total.toFixed(
                                        2
                                      )}
                                      {selectedOrder.status === "cancelled" && (
                                        <span className="ml-2 text-xs text-red-600">
                          ...
                          {/* Cancellation and Refund Information */}
                          {selectedOrder.status === "cancelled" &&
                            (selectedOrder as any).refundDetails && (
                                ...
                                <CardContent className="space-y-4">
                                  ...
                                  <div className="mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-300">
                                    <p className="text-sm text-yellow-800">
                                      <strong>Note:</strong> Refund has been
                                      processed through Razorpay and will appear
                                      in the customer's account within 5-7
                                      business days.
                                    </p>
                                  </div>
                                </CardContent>
                            ...
                            <Card>
                              ...
                              <CardContent className="space-y-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-gray-900">
                                    ₹
                                    {selectedOrder.paymentDetails.amount.toFixed(
                                      2
                                    )}{" "}
                            ...
                            <Card>
                              ...
                              <CardContent className="space-y-3">
                                <div className="flex justify-between text-sm">
                                  ...
                                  <span className="font-medium">
                                    ₹
                                    {selectedOrder.orderSummary.subtotal.toFixed(
                                      2
                                    )}
                                  </span>
                                </div>
                                <div className="flex justify-between text-sm">
                                  ...
                                  <span className="font-medium">
                                    {selectedOrder.orderSummary.shipping === 0
                                      ? "Free"
                                      : `₹${selectedOrder.orderSummary.shipping.toFixed(
                                          2
                                        )}`}
                                  </span>
                                </div>
                                <div className="flex justify-between text-sm">
                                  ...
                                  <span className="font-medium">
                                    ₹
                                    {selectedOrder.orderSummary.taxes.toFixed(
                                      2
                                    )}
                                  </span>
                                </div>
                                  ...
                                  <div className="flex justify-between text-sm">
                                    <span className="text-green-600">
                                      ...
                                      {selectedOrder.orderSummary
                                        .discountCode &&
                                        `(${selectedOrder.orderSummary.discountCode})`}
                                    </span>
                                    <span className="text-green-600 font-medium">
                                      -₹
                                      {selectedOrder.orderSummary.discount.toFixed(
                                        2
                                      )}
                                    </span>
                                  </div>
